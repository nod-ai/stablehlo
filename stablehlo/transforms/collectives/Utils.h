...
inline FailureOr<StringAttr> makeHloShardingStrAttr(
  ...
  auto shardingStrPtr = std::unique_ptr<char[], xla::api::DestroyCharBuffer>(
      shardingStr, xla::api::destroyCharBuffer);
...
inline FailureOr<ArrayAttr> makeHloShardingArrayAttr(
    const xla::HloSharding& sharding, Location loc) {
  if (!xla::api::hloShardingIsTiled(&sharding)) {
...
inline FailureOr<Attribute> makeHloShardingAttr(
  ...
  if (expandTuple && xla::api::hloShardingIsTiled(&sharding)) {
    ArrayAttr arrayAttr;
