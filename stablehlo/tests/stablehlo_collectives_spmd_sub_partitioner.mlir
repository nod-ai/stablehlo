...
//              CHECK: %[[POST_SHARDING_RES:[A-Za-z0-9]+]] = stablehlo.custom_call @Sharding(%[[ALL_REDUCE_RES]]) {mhlo.sharding = "{manual}"} : (tensor<3x2xf32>) -> tensor<3x2xf32>
...
//              CHECK: %[[PRE_SHARDING_RES:[A-Za-z0-9]+]] = stablehlo.custom_call @Sharding(%[[ARG0]]) {mhlo.sharding = "{devices=[3,1,1]0,1,2}"} : (tensor<9x4x2xf32>) -> tensor<9x4x2xf32>
//              CHECK: %[[FULL_TO_SHARD_SHAPE_RES:[A-Za-z0-9]+]] = stablehlo.custom_call @SPMDFullToShardShape(%[[PRE_SHARDING_RES]]) {mhlo.sharding = "{manual}"} : (tensor<9x4x2xf32>) -> tensor<3x4x2xf32>
...
//              CHECK: %[[POST_SHARDING_RES:[A-Za-z0-9]+]] = stablehlo.custom_call @Sharding(%[[ALL_TO_ALL_RES]]) {mhlo.sharding = "{manual}"} : (tensor<3x2x4xf32>) -> tensor<3x2x4xf32>
//              CHECK: %[[SHARD_TO_FULL_SHAPE_RES:[A-Za-z0-9]+]] = stablehlo.custom_call @SPMDShardToFullShape(%[[POST_SHARDING_RES]]) {mhlo.sharding = "{devices=[3,1,1]0,1,2}"} : (tensor<3x2x4xf32>) -> tensor<9x2x4xf32>
