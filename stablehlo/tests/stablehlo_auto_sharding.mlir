...
// CHECK{LITERAL}:   func.func @main(%arg0: tensor<16x16xi32> {mhlo.sharding = "{devices=[4,1,2]0,4,1,5,2,6,3,7 last_tile_dim_replicate}"}, %arg1: tensor<16x16xi32> {mhlo.sharding = "{devices=[1,2,4]0,1,2,3,4,5,6,7 last_tile_dim_replicate}"}) -> (tensor<16x16xi32> {mhlo.sharding = "{devices=[4,2]0,4,1,5,2,6,3,7}"}) {
// CHECK{LITERAL}:     %0 = stablehlo.dot %arg0, %arg1, precision = [DEFAULT, DEFAULT] {mhlo.sharding = "{devices=[4,2]0,4,1,5,2,6,3,7}"} : (tensor<16x16xi32>, tensor<16x16xi32>) -> tensor<16x16xi32>
// CHECK{LITERAL}:     %1 = stablehlo.add %arg0, %0 {mhlo.sharding = "{devices=[4,2]0,4,1,5,2,6,3,7}"} : tensor<16x16xi32>
